...
disp_income %>% 
  ...
  fmt_currency( # Reformat to currency notation
    columns = vars(dpi), # The values for 'dpi' variable
...
```{r}
# Get some summary stats from the single salinity sample:
hist(salinity$sal)
```
```{r}
mean(salinity$sal)
...
# The histogram of the originial sample:
...
# Model linear to get *k* estimate (the slope of this linear equatio is an estsimate of the growth rate constant):
...
model_out <- broom::tidy(df_nls) # View(model_out)

# Want to just get one of these?
A_est <- model_out$estimate[2] # Get first row in estimate column
```
...
```{r}
# Make predictions for the population at tall of those times in the originial df
p_predict <- predict(df_nls)

# Bind predictions to originial data frame
